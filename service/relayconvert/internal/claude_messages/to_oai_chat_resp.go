...
func FormatClaudeResponseInfo(claudeResponse *dto.ClaudeResponse, oaiResponse *dto.ChatCompletionsStreamResponse, claudeInfo *ClaudeResponseInfo) bool {
	...
	if claudeResponse.Type == "message_start" {
		...
		if claudeResponse.Delta != nil {
			...
			claudeInfo.HasSemanticOutput = claudeInfo.HasSemanticOutput ||
				(claudeResponse.Delta.PartialJson != nil && *claudeResponse.Delta.PartialJson != "")
		...
		if claudeResponse.ContentBlock != nil {
			...
			case "thinking":
				claudeInfo.HasSemanticOutput = claudeInfo.HasSemanticOutput ||
					(claudeResponse.ContentBlock.Thinking != nil && *claudeResponse.ContentBlock.Thinking != "")
			case "fallback", "":
				// fallback 是模型切换标记，空类型不是有效内容。
			default:
				// 工具调用、服务端工具结果和未来新增内容块均属于有效输出。
				claudeInfo.HasSemanticOutput = true
			}
		}
	} else {

...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo types.GroupRatioInfo, channelRatio float64) (types.PriceData, error) {
	...
	estimatedQuotaAfterGroup, err := billingexpr.QuotaRoundStrict(quotaBeforeGroup * groupRatioInfo.GroupRatio)
	if err != nil {
		return types.PriceData{}, err
	}
	preConsumedQuota, err := common.QuotaRoundStrict(float64(estimatedQuotaAfterGroup) * channelRatio)
	if err != nil {
		return types.PriceData{}, err
	}

...
- Soft-failure cooldown routing is gated by `channel_health_setting.soft_failure_cooldown_enabled` and applies to Claude `/v1/messages` and validated Twork `model-routes-v3` model-mode `/v1/responses`; when disabled, selection, affinity, retry limits, and error recording must preserve their legacy behavior. Specific-channel requests and channel tests bypass both cooldown filtering and recording.
- With soft-failure cooldown enabled, Claude `/v1/messages` affinity rebinding requires `ContextKeyClaudeRelaySucceeded`, set only after relay success while the client request is still active; an HTTP 200 or committed SSE stream alone is not success. Twork model-mode requests require `ContextKeyTworkRelaySucceeded`; other non-Claude requests keep the legacy HTTP-status check.
- Twork 普通 Responses 选路可在临时故障时覆盖亲和规则的 `skip_retry_on_failure`；固定渠道、旧 Codex 有状态请求及 compact 不参与。失败流不能标记成功；已提交内容后只发送 SSE 错误，保留已输出用量结算并将切换延至下一次请求。
- Responses 冷却按渠道和模型隔离，不写永久禁用状态；冷却结束每 30 秒最多一次真实请求试探，成功仅清理同一故障代次，已有健康备用渠道的会话不抢回高优先级渠道。
- Cooldown filtering is a request-local exclusion layer after the existing group/model/path/token/permanent-disable filters. It must not mutate `RetryParam.ExcludedChannelIds` or consume a real-attempt budget; initial all-cooling exhaustion returns the existing 503 body with `Retry-After`, while retry exhaustion preserves `RelayInfo.LastError`.
- Soft-cooldown observability is nested under `other.admin_info.soft_cooldown`; Prometheus labels stay low-cardinality (`channel_id`, classified error, fixed outcome, cache operation). Failover outcomes are mutually exclusive per request: same-request success, deferred after semantic commit, exhausted, or all cooling.
- The expanded Claude soft-cooldown retry budget applies only when the latest relay error is classified as a soft model-health failure; every non-soft failure still uses `common.RetryTimes`, even inside a request that previously had a soft failure.
- The Claude soft-cooldown loop starts with the legacy `common.RetryTimes + 1` attempt cap and expands it only after the request has actually observed a soft model-health failure; `channel:*` errors alone must never activate the expanded cap.
- Soft-cooldown cache reads/writes use the incoming request context with a 200 ms timeout. The first cache error opens a request-local circuit so later cooldown accesses fail open without touching Redis again; affinity cache calls keep their existing timeout behavior.
- Soft-cooldown `error_class` values are limited to `rate_limit`, `overloaded`, `server_error`, and `unknown`. `admin_info.soft_cooldown.applied` means the cache write succeeded; affinity clearing alone still makes the request eligible for a failover outcome.
...
- 携带有效正式版 `X-Twork-Client-Version >= 4.0.0` 的请求按公开模型 ID 隔离：Claude 仅允许 Anthropic（14），其他模型（含 Auto、summarization-model）仅允许非 Anthropic；过滤必须先于优先级、权重、粘性、固定渠道和重试，缺失版本头保持旧版兼容，旧协议仍沿用原缓存、粘性与重试路径；不能因版本头把所有旧请求切到数据库选路。模型级选路得到独立渠道对象，多密钥轮询仍必须在渠道锁内推进共享游标。

...
const maxChannelConsumptionRangeSeconds int64 = 31 * 24 * 60 * 60
...
func GetChannelConsumption(c *gin.Context) {
	...
	if endTimestamp-startTimestamp > maxChannelConsumptionRangeSeconds {
		common.ApiErrorMsg(c, "time range cannot exceed 31 days")
		return
	}

	data, err := model.GetChannelConsumptionData(startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
